...
from llama_index import Document as LlamaDoc
...
def chunk_content_helper(
    ...
    #llama_doc = LlamaDoc(text=doc.content, metadata={"title": doc.title})
    ...
    if file_format == "json" or doc_content_size < num_tokens:
        ...
        if file_format == "markdown":
            ...
            #nodes = parser.get_nodes_from_documents(documents=[llama_doc])
            
            for chunked_content in chunked_content_list:
#                chunked_content = node.text
                chunk_size = TOKEN_ESTIMATOR.estimate_tokens(chunked_content)
